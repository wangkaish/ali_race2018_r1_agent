...
public final class EtcdRegistry {
...
//            logger.info(ThrowableUtil.stackTraceToString(e));
    ...
    private void modifyEndpoint(KeyValue kv, boolean add) {
        NioEventLoopGroup group = AgentApp.consumerAgentGroup;
...
//        logger.info("build endpoint:{}",endpoint);
        for (int i = 0; i < group.getEventLoopSize(); i++) {
            if (add) {
                EndpointGroup.registEndpoint(group.getEventLoop(i), endpoint.clone());
            } else {
                EndpointGroup.deRegistEndpoint(group.getEventLoop(i), endpoint.clone());
            }
        }
    }
